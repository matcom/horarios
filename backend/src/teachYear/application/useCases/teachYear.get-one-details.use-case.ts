...
export type FindDetailsTeachYearUseCaseResponse =
...
@Injectable()
export class FindDetailsTeachYearUseCase implements IUseCase<{ id: string }, Promise<FindDetailsTeachYearUseCaseResponse>> {

  private _logger: Logger;

  constructor(private readonly teachYearRepository: TeachYearRepository) {
    this._logger = new Logger('FindDetailsTeachYearUseCase');
  }

  async execute(request: {
    id: string
  }): Promise<FindDetailsTeachYearUseCaseResponse> {
    try {
      return Optional(await this.teachYearRepository.findDetails(request.id))
