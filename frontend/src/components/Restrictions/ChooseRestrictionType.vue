  ...
  <div>
    <div role='document'>
      ...
      <div class='modal-body'>
        <div class='' v-for='(it, index) in restrictionType' :key='it'>
          <div class='bg-white'>
            <input type='checkbox' data-toggle='collapse' href='#type_description' role='button' aria-expanded='false'
                   aria-controls='type_description' v-model='it.isSelected' @click='disableOthers(it.name)'>
            <span class='ml-2'>{{ it.name }}</span>
          </div>
          <div class='collapse' id='type_description'>
