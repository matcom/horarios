    ...
    <div class='row'>
      <div class='col-12'>
        <div class='card mb-1 bg-white border-bottom-primary'>
          <a href='#collapseCardTypesEvents' class='d-block card-header py-3' data-toggle='collapse' role='button'
             aria-expanded='false' aria-controls='collapseCardTypesEvents'>
          ...
          <div class='collapse hide' id='collapseCardTypesEvents' style=''>
            <div class='card-body p-2'>
              <div class='row justify-content-center'>
                <div class='col-10'>
                  <input type='text' v-model='type_events' class='form-control bg-light border-0 small'
        ...
        <div class='card'>
          <div class='card-body p-0'>
            <div class='list-group'>
              <button v-if='tag.events.length === 0' type='button' class='list-group-item list-group-item-action'
                      disabled>El tipo no tiene ningún evento asignado
              </button>
              <button v-else-if="filterList(tag.events, type_events, 'title').length === 0" type='button'
                      class='list-group-item list-group-item-action' disabled>No hay resultados para mostrar
              </button>
              <router-link v-for="event in filterList(tag.events, type_events, 'title')" :key='event.id'
