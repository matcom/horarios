        ...
        <div class='card'>
          <div class='card-body p-0'>
            <div class='list-group'>
              <button v-if="filterList(groups, text, 'name').length === 0" type='button'
                      class='list-group-item list-group-item-action' disabled>No hay resultados para mostrar
              </button>
              <router-link v-for="group in filterList(groups, text, 'name')" :key='group.id'
                           :to="{name: 'editorPage', params: {groupId: group.id}}"
