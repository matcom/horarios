  ...
  <div id='group'>
    <div class='row'>
      <div class='col-12'>
        ...
        <div v-for='(noti, index) in notifications' :key='noti.id' class='card mb-2'
             :class="noti.seened ? '' : 'text-white bg-primary'">
          <div class='card-body'>
            <h5 class='card-title'><strong>{{ noti.title }}</strong></h5>
            <h6 class='card-subtitle mb-1' :class="noti.seened ? 'text-muted' : 'text-white'"
                v-if='noti.groups.length > 0'>Grupo(s): {{ parseGroupsToStr(noti.groups) }}</h6>
            <p class='card-text'>{{ noti.body }}</p>
            <a href='' class='card-link stretched-link' :class="noti.seened ? 'text-muted' : 'text-white'"
               @click.prevent='seen(noti.id, index)'>Fecha: {{ render_date(noti.date) }}</a>
          </div>
        </div>
      </div>
    </div>
