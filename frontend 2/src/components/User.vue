    ...
    <div class='row'>
      <div class='col-12'>
        <div class='card mb-1 bg-white border-bottom-primary'>
          <a href='#collapseCardUsersGroups' class='d-block card-header py-3' data-toggle='collapse' role='button'
             aria-expanded='false' aria-controls='collapseCardUsersGroups'>
          ...
          <div class='collapse hide' id='collapseCardUsersGroups' style=''>
            <div class='card-body p-2'>
              <div class='row justify-content-center'>
                <div class='col-10'>
                  <input type='text' v-model='user_groups' class='form-control bg-light border-0 small'
        ...
        <div class='card'>
          <div class='card-body p-0'>
            <div class='list-group'>
              <button v-if='user.groups.length === 0' type='button' class='list-group-item list-group-item-action'
                      disabled>El usuario no tiene ningún grupo asignado
              </button>
              <button v-else-if="filterList(user.groups, user_groups, 'name').length === 0" type='button'
                      class='list-group-item list-group-item-action' disabled>No hay resultados para mostrar
              </button>
              <router-link v-for="group in filterList(user.groups, user_groups, 'name')" :key='group.id'
