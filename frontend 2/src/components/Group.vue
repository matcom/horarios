    ...
    <div class='row'>
      <div class='col-6'>
        <div class='card mb-1 bg-white border-bottom-primary'>
          <a href='#collapseCardGroupsEvents' class='d-block card-header py-3' data-toggle='collapse' role='button'
             aria-expanded='false' aria-controls='collapseCardGroupsEvents'>
          ...
          <div class='collapse hide' id='collapseCardGroupsEvents' style=''>
            <div class='card-body p-2'>
              <div class='row'>
                <div class='col-9'>
                  <input type='text w-100' v-model='group_events' class='form-control bg-light border-0 small'
        ...
        <div class='card'>
          <div class='card-body p-0'>
            <div class='list-group'>
              <button v-if='group.events.length === 0' type='button' class='list-group-item list-group-item-action'
                      disabled>El grupo no tiene ningún evento asignado
              </button>
              <button v-else-if="filterList(group.events, group_events, 'title', event_val).length === 0" type='button'
                      class='list-group-item list-group-item-action' disabled>No hay resultados para mostrar
              </button>
              <router-link v-for="event in filterList(group.events, group_events, 'title', event_val)" :key='event.id'
      ...
      <div class='col-6'>
        <div class='card mb-1 bg-white border-bottom-primary'>
          <a href='#collapseCardGroupsUsers' class='d-block card-header py-3' data-toggle='collapse' role='button'
             aria-expanded='false' aria-controls='collapseCardGroupsUsers'>
          ...
          <div class='collapse hide' id='collapseCardGroupsUsers' style=''>
            <div class='card-body p-2'>
              <div class='row'>
                <div class='col-9'>
                  <input type='text w-100' v-model='group_users' class='form-control bg-light border-0 small'
                         placeholder='Buscar ...' aria-label='Search' aria-describedby='basic-addon2'>
                </div>
                <div class='col-3'>
        ...
        <div class='card'>
          <div class='card-body p-0'>
            <div class='list-group'>
              <button v-if='group.users.length === 0' type='button' class='list-group-item list-group-item-action'
                      disabled>El grupo no tiene ningún usuario asignado
              </button>
              <button v-else-if="filterList(group.users, group_users,'username', user_val).length === 0" type='button'
                      class='list-group-item list-group-item-action' disabled>No hay resultados para mostrar
              </button>
              <router-link v-for="user in filterList(group.users, group_users,'username', user_val)" :key='user.id'
                           :to="{name: 'userPage', params: {userId: user.id}}"
                           class='list-group-item list-group-item-action'>{{ user.username }}
              </router-link>
            </div>
          </div>
        </div>
